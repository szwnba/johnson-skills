# 📜 Deep Research — 横纵分析法深度研究报告

> 自研 prompt。原文是 ZCode 的自定义命令 `~/.zcode/commands/Deep-Research.md`（触发词 `/deep-research`）。

## 用法

1. **在 ZCode 里用**：复制本文件全文到 `~/.zcode/commands/Deep-Research.md`，输入 `/Deep-Research` + 研究对象即可触发。
2. **任何 Agent 里用**：把「变量定义」一节的研究对象替换成你要研究的名字，全文贴进对话。

## 适用对象

产品/工具（Cursor、Claude Code）、公司/组织（Anthropic）、技术概念（MCP 协议、RAG）、人物（行业关键人物的职业轨迹）。

## 效果基准

实测产出 1~3 万字长文，结构为：纵向发展史叙事（主体）→ 横向竞品对比 → 横纵交汇总结。已用它产出 AI 测试赛道两份报告（AI 直测 WebUI、Playwright CLI 选型）。
